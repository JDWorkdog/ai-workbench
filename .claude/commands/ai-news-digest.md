...
Determine today's journal file: `personal/journal/<YYYY>/YYYY-MM-DD-DAY.md` (dailies live in year folders; see `personal/journal/SCHEMA.md`). Verify the date per the workspace date rules before naming the file.

- If the file **exists**: append the digest at the end, after the canonical sections, as two extra sections: `## AI Feature Releases` and `## Draft X Posts`. Extend the frontmatter `topics` list with `ai-news` and add `web` to `sources`.
- If the file **does not exist**: create it per the schema (full frontmatter, canonical header), then append the digest sections. Canonical sections with nothing in them are omitted per the schema:
...
If there were draft posts, note: "Review the drafts above, then copy and personalize any you want to post on X today."
