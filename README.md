...
- **A model-routing layer.** A five-role delegation roster with pinned models (`.claude/agents/` and `.codex/agents/`), a routing table, and a QA policy, so frontier models manage and cheap tiers do the mechanical work. Method in [`guides/model-routing-guide.md`](guides/model-routing-guide.md). Want just this layer in your own projects without the rest of the workbench? See the [routing quickstart](guides/routing-quickstart.md); the `tune-my-harness` skill is self-contained and cherry-pickable.
